...
the same and the client is the same; what changes is that the four things wired by hand here come
from that gem instead, and `config.slots = :client` is the whole configuration. Comparing them is
the clearest way to see what it is for.
